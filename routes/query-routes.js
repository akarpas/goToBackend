...
function checkFlightsMaxPrice(query, resultsFirstFilter, cb ) {
  ...
  async.each(resultsFirstFilter, (item, callback)=>{
    ...
    const url = "https://api.sandbox.amadeus.com/v1.2/flights/low-fare-search?apikey="+process.env.AMADEUS_KEY+"&origin="+origin+"&destination="+destination+"&departure_date=" + outDate + "&return_date=" + inDate + "&adults=1&max_price=" + flightCost + "&currency=" + currency + "&number_of_results=" + numberOfResults;
